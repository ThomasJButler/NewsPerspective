...
- Updated on 2026-03-12 after a frontend build slice across `AGENTS.md`, `IMPLEMENTATION_PLAN.md`, active frontend specs/source, and the focused frontend validation commands.
- Priority guide:
...
## 2. Active phase
Phase 4 closeout: the backend validation-isolation and SQLite lifecycle warnings are closed, and the refresh completion toast now distinguishes added-new count from processed count. The next loop should take the remaining frontend spec drift and then the router review cleanup. Phase 3 trusted-machine evidence is current, and no new runtime contract break was found in the app flow.
...
Take the frontend spec-alignment slice:

1. Update `specs/FRONTEND.md` so the Refresh UX section explicitly documents the shipped timeout-reset / same-refresh resume behavior after the 120-second polling timeout.
2. Keep the spec wording aligned with the current `src/frontend/app/page.tsx` and `src/frontend/lib/refresh-status.ts` behavior rather than restating roadmap intent.
3. Re-read the affected spec section after editing, then follow with the `specs/ROADMAP.md` near-term ordering cleanup in the next loop.
