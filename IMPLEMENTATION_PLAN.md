...
## Active status
- Updated on 2026-03-10 after re-aligning `specs/FRONTEND.md` with the current v2 home-route, refresh, polling, Docker, and validation behavior.
- Fully completed work was moved to `specs/completedarchive/2026-03-10-implementation-completed-archive.md`.
- Active phase: Phase 3D. The remaining work is trusted-machine evidence, refresh-path browser coverage, header branding, and the remaining backend policy/test decisions.
...
- [x] Update `specs/FRONTEND.md` for Next.js `16.1.6`, `next.config.ts`, the inline onboarding flow, refresh UX, refresh polling, Docker local flow, and current validation caveats.
...
## Validation notes
- Re-ran on 2026-03-10 after the `specs/FRONTEND.md` alignment slice and article-detail lint cleanup: `cd src/frontend && npm run lint`, `cd src/frontend && npm run typecheck`, and `cd src/frontend && npx playwright test tests/e2e/cached-browse.spec.ts` all passed.
- Last confirmed on 2026-03-10 for the `specs/FRONTEND.md` docs/spec slice: content was cross-checked against `src/frontend/package.json`, `src/frontend/next.config.ts`, `src/frontend/app/page.tsx`, `src/frontend/app/article/[id]/page.tsx`, `src/frontend/components/header.tsx`, `src/frontend/components/api-key-setup.tsx`, `src/frontend/components/settings-dialog.tsx`, `src/frontend/components/article-card.tsx`, `src/frontend/components/article-feed.tsx`, `src/frontend/components/stats-bar.tsx`, `src/frontend/hooks/use-api-key.ts`, `src/frontend/lib/api.ts`, `src/frontend/playwright.config.mts`, `src/frontend/compose.yaml`, `src/frontend/scripts/docker-start-app.sh`, and `src/frontend/tests/e2e/cached-browse.spec.ts`.
...
- Step 18.1 if running on a trusted local machine with a real `NEWS_API_KEY`: gather the missing Phase 3 refresh evidence and record the exact observed outcomes in this plan. Otherwise, take the current P2 browser-coverage gap by adding deterministic Playwright coverage for accepted refresh, invalid-key UX, refresh-status polling, and duplicate-refresh messaging.
