...
- Updated on 2026-03-12 after closing the trusted-machine key-handling/doc contract in the manual-evidence helper and repo docs, while keeping the earlier local review-finding fixes in place across the backend and refresh-path fixtures.
...
- Fresh validation rerun during this implementation pass:
  ...
  - Docs/helpers: targeted `rg` checks confirmed the stale `--api-key "$NEWS_API_KEY"` helper examples and direct `curl -H "X-News-Api-Key: $NEWS_API_KEY"` guidance are gone from the touched files.
...
## 2. Active phase
Phase 3D remains active. The local P1 review findings, the remaining P2 refresh-path fixture drift, and the trusted-machine key-handling/doc contract are now closed in the worktree. The next priority is the broader doc/spec alignment slice, and only then the real local evidence pass. Keep the 2026-03-10 v2 boundary intact: if legacy v1 behavior needs reference, recover it from git history or `READMEOLD.md` instead of recreating root-level runtime files.
...
- Land the remaining doc/spec alignment slice next in `specs/BACKEND.md`, `src/frontend/README.md`, `AGENTS.md`, and `specs/OVERVIEW.md`.
- After that, finish the trusted-machine Phase 3 evidence pass on a real local machine and paste the concrete observed outcomes back into this file.
