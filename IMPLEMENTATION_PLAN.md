...
## Active status
- Updated on 2026-03-10 after making `READMEOLD.md` legacy-only and re-aligning `specs/OVERVIEW.md` with the current v2 runtime.
...
- [P1] Top-level docs and specs still drift from the running app, especially `AGENTS.md`, `specs/BACKEND.md`, and `specs/FRONTEND.md`.
...
- Step 18.1 if running on a trusted local machine with a real `NEWS_API_KEY`: gather the missing Phase 3 refresh evidence and record the exact observed outcomes in this plan. Otherwise, continue the remaining docs/spec alignment work with `specs/BACKEND.md`.
